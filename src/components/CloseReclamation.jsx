...
export default function CloseReclamation({ onCloseAction, record }) {
    ...
    const isClosed = !!record?.closing_date;
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            message.error(
                "Une erreur est survenue lors de la clôture de l'action."
            );
        }
    };
    ...
    return (
        ...
        >
            <Tooltip title="Clôture">
                <Button
                    disabled={
                        !permissions("cloturer.reclamation") ||
                        Number(record?.open_corrective_actions_count) > 0 ||
                        isClosed
                    }
                    size="small"
                    icon={<SendOutlined />}
