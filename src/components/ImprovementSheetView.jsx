...
// Renders a date professionally: absolute date + weekday + relative distance,
// with a subtle icon and muted secondary line. Falls back to a dash when empty.
...
export default function ImprovementSheetView({ id, onEdit }) {
    ...
    if (!sheet) {
        return <Empty description="Fiche introuvable." className="py-20" />;
    }
        ...
        <div className="px-3">
            <div className="flex justify-between items-start mt-3 mb-4">
                <div>
                    <Space align="center" size={10}>
                        ...
                        <Tag color="default">{sheet.code}</Tag>
                        <Tag color={STATUT_COLORS[sheet.statut] || "default"}>
                            {sheet.statut}
                        </Tag>
                    </Space>
                    <div className="text-xs text-slate-400 mt-1">
                        ...
                        {sheet.corrective_action?.code && (
                            <>
                                {" · "}
                                <FileTextOutlined /> {sheet.corrective_action.code}
                            </>
                        )}
                    </div>
                </div>
                         ...
                         <Row gutter={24}>
                            <Col span={12}>
                                <Field label="Créée le">
                                    <DateDisplay value={sheet.created_at} />
                                </Field>
                            </Col>

                            <Col span={12}>
                                <Field label="Dernière mise à jour">
                                    <DateDisplay value={sheet.updated_at} />
                                </Field>
                            </Col>
                        </Row>
                    </Card>
                        ...
                        <Row gutter={24}>
                            <Col span={12}>
                                <Field label="Date d'évaluation">
                                    <DateDisplay value={sheet.evaluation_date} />
                                </Field>
                            </Col>

                            <Col span={12}>
                                <Field label="Date de clôture">
                                    <DateDisplay value={sheet.closing_date} />
                                </Field>
                            </Col>
                        </Row>
            ...
            <Card size="small" variant>
                ...
                {sheet.improvement_actions?.length > 0 ? (
                            ...
                            >
                                <Space>
                                    ...
                                    <div className="leading-tight">
                                        ...
                                        <Text type="secondary" className="text-xs">
                                            {r.service?.name || "Processus non défini"}
                                        </Text>
                                    </div>
                                </Space>
                            </div>
                    ...
                    <Text type="secondary">Aucun responsable assigné.</Text>
                )}
            </Card>
